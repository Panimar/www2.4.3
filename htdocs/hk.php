...
print $box_simple_wide->toString();
    ...
    //(c) AXE
	 $cont2_title='<center>Top Kills for the '.$rname.' Realm</center>';
	...
	if ($server_core=='trinity' or $server_core=='trinity_ra')
	{
			...
			$SQLawow ="SELECT * from ".$db_translation['characters']." where ".$db_translation['characters_honorPoints']."<>'0' order by ".$db_translation['characters_killsLifeTime']." DESC"; 
			
			// no banned and no 0 kills
			$char=mysql_query($SQLawow) or error('Something is wrong with the database. <br><br></strong>MySQL reported:<strong> '.mysql_error().'.<br><br></strong>Suggestion:<strong> Please open your "config.php" file, find variable $realm['.$realmid.'], and enter the correct database name', __FILE__, __LINE__); 
...
//                          die($SQLwow);
                            $SQLwow2=mysql_query($SQLwow) or error('Something is wrong with the database. <br><br></strong>MySQL reported:<strong> '.mysql_error().'<br><br>'.$SQLwow, __FILE__, __LINE__);
	...
	{	$SQLawow ="SELECT * from ".$db_translation['characters']." where ".$hk_where." ".$db_translation['characters_honorPoints']."<>'0' order by ".$db_translation['characters_killsLifeTime']." DESC"; 
			...
			// no banned and no 0 kills
			$char=mysql_query($SQLawow) or error('Something is wrong with the database. <br><br></strong>MySQL reported:<strong> '.mysql_error().'.<br><br></strong>Suggestion:<strong> Please open your "config.php" file and find the variable $realm['.$realmid.']. Then enter the correct database name', __FILE__, __LINE__); 
			while ($char2=mysql_fetch_array($char) ){
			 if ($a<=19)
					 {     
					 		mysql_select_db($acc_db);
	                        ...
	                        $SQLwow2=mysql_query($SQLwow) or error('Something is wrong with the database. <br><br></strong>MySQL reported:<strong> '.mysql_error().'.<br><br></strong>Suggestion:<strong> Please open your "config.php" file, find the variable $acc_db, and enter the correct database name', __FILE__, __LINE__); 
							$SQLwow3=mysql_fetch_array($SQLwow2);
